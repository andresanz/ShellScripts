...
##
WWW_TRANSFORM='s,^etc/apache2/$DOMAIN_NAME,www,'
...
sudo tar -cvf $BACKUP_DIR/$DOMAIN_NAME/$FILE --transform $WWW_TRANSFORM $WWW_DIR
...
sudo gzip -9 $BACKUP_DIR/$DOMAIN_NAME/$FILE

##
## Erase backups older than 7 days
##
find $BACKUP_DIR/$DOMAIN_NAME -type f -mtime +$ARCHIVEHOLD -name '*.gz' -execdir rm -- '{}' \;
...
## Send logfile
##
mail -s "BackupLog $TYPE $DOMAIN_NAME" sanz.andre@gmail.com < /home/ubuntu/scripts/logs/BackupLog_$DOMAIN_NAME.log
